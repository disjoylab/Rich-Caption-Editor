...
public class CaptionRenderer : MonoBehaviour
{
    ...
    private void ResolveRegionElement()
    {
        foreach (Style style in ProjectManager.Instance.CurrentRCEProject.GetCurrentStyleGroup().Styles)
        {
            if (style.IsDefault(myCueGroup.Name))
            {
                SetRegionFromFeature(style.GetFeature());
            }
        }
        SetRegionFromFeature(FeatureManager.GetFeature(CurrentCue.RegionFeature));
    }

    private void SetRegionFromFeature(Feature feature)
    {
        if (feature == null)
        {
            return;
    ...
    private void ResolveCueElements()
    {
        foreach (Style style in ProjectManager.Instance.CurrentRCEProject.GetCurrentStyleGroup().Styles)
        {
            if (style.element.Match(CurrentCue.CueElement) || style.IsDefault(myCueGroup.Name))
            {
                Feature feature = style.GetFeature();
                if (feature != null)
                {
                    SetCueFromFeature(feature);
                }
            }
        }
    }

    private void SetCueFromFeature(Feature feature)
    {
        if (feature == null)
        {
            return;
    ...
    private void ResolveCueCharSettings()
    {
        ...
        foreach (var cueChar in CurrentCue.GetCurrentTextSegment().Content)
        {
            ...
            foreach (Style cueGroupStyle in currentStyleGroup.Styles)
            {
                List<Element> elements = new List<Element>();
                elements.AddRange(cueChar.elements);
                if (cueGroupStyle.element.Match(elements) || cueGroupStyle.IsDefault(myCueGroup.Name))
                {
